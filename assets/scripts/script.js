...
function openDialog(dialog) {
    document.getElementById("m-cont").style.display = "none";
    dialog.show();
    backdrop.style.display = 'block';
}

function closeDialog() {
    document.getElementById("m-cont").style.display = "flex";
    serviceModal.close();
    projectsModal.close();
    contactModal.close();
...
//    "<div class='g-item'><img src='' alt='website'><a href=''></a></div>",
...
gItems.forEach(item => {
    gPrev.innerHTML += `<img id="prev${item.id}" class="prev" src=${item.src} alt="website">`;
});

let post = 0;
let gallery = document.querySelector("#gallery")
const t = document.querySelector(".testim")


document.addEventListener('DOMContentLoaded', () => {
    document.getElementById("prev" + post).classList.add("active");
    item = gItems[post];
    gallery.innerHTML = `<div id="img${item.id}" class='g-item'><img src="${item.src}" alt='website'><a href='${item.web}'>${item.web}</a></div>`;
});
